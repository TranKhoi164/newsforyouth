      ...
      <div className={classes.createPost_form}>
        <form onSubmit={handleSubmit} style={{marginTop: '0', width: '80%'}}>
          ...
          <div className={classes.input_container}>
            ...
            <textarea required onChange={handleChange} placeholder='Nội dung' name='content' id='content' className={classes.input_field} value={post.content}  />
          </div>
          <div className={classes.input_container}>
            <label htmlFor="categorySlug">Danh mục (không dấu, có gạch ngang giữa các chữ vd: thoi-su, am-thuc, ...)</label>
            <input value={post.categorySlug} required type="text" onChange={handleChange} placeholder='Danh mục' name='categorySlug' id='categorySlug' className={classes.input_field} />
          </div>

          {/* upload image */}
          <FileUpload post={post} setPost={setPost} setErr={setErr} />
          <div className={classes.submit_container}>
            <button type='submit' className={classes.submit_btn}>SỬA BÀI VIẾT</button>
          </div>
        </form>
      </div>
    </div>
